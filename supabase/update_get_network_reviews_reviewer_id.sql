-- Migration: Add reviewer_id to get_network_reviews and search_network_reviews return columns
-- 1. Recreate get_network_reviews function with reviewer_id included
DROP FUNCTION IF EXISTS get_network_reviews(uuid, integer, text) CASCADE;
...
BEGIN
  ...
  SELECT g.review_id, g.reviewer_id, g.reviewer_name, g.reviewer_pseudonym, g.is_anonymous, g.item_title, g.item_type, g.item_poster_url, g.item_release_year, g.rating, g.comment, g.depth, g.created_at, g.updated_at, g.item_id
  FROM (
...
-- 2. Recreate search_network_reviews function to match
...
BEGIN
  RETURN QUERY
  SELECT gnr.review_id, gnr.reviewer_id, gnr.reviewer_name, gnr.reviewer_pseudonym, gnr.is_anonymous, gnr.item_title, gnr.item_type, gnr.item_poster_url, gnr.item_release_year, gnr.rating, gnr.comment, gnr.depth
