-- Migration: Add liked and disliked columns to reviews table and update network query functions
-- 1. Alter reviews table
ALTER TABLE reviews 
  ADD COLUMN IF NOT EXISTS liked TEXT[] DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS disliked TEXT[] DEFAULT '{}';

-- 2. Drop and recreate get_network_reviews with liked & disliked columns
...
-- 3. Drop and recreate search_network_reviews to match
...
BEGIN
  ...
  SELECT 
    ...
    gnr.reviewer_id, 
    gnr.reviewer_name, 
    gnr.reviewer_pseudonym, 
    gnr.is_anonymous, 
    gnr.item_title, 
    gnr.item_type, 
    gnr.item_poster_url, 
    gnr.item_release_year, 
    gnr.rating, 
    gnr.comment, 
    gnr.depth,
    gnr.liked,
